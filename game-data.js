const stationsDataRaw = `1 190 40 taxi,bus,underground
2 487 20 taxi
3 675 25 taxi,bus
4 790 15 taxi
5 1253 30 taxi
...
const connectionsDataRaw = `108 115 water
115 157 water
157 194 water
1 46 underground
13 46 underground
...
stationsDataRaw.split('\n').forEach(line => {
    ...
    stations[id] = {
        ...
        x: parseInt(parts[1]),
        y: parseInt(parts[2]),
        connections: { taxi: [], bus: [], underground: [], ferry: [] }
    };
});

connectionsDataRaw.split('\n').forEach(line => {
    const parts = line.split(' ');
    if (parts.length < 3) return;
    const [from, to, type] = parts;
    const fromId = parseInt(from);
    const toId = parseInt(to);
    const transportType = type === 'water' ? 'ferry' : type;

    if (stations[fromId] && stations[toId]) {
        if (!stations[fromId].connections[transportType]) {
            stations[fromId].connections[transportType] = [];
        }
        stations[fromId].connections[transportType].push(toId);

        if (!stations[toId].connections[transportType]) {
            stations[toId].connections[transportType] = [];
        }
        stations[toId].connections[transportType].push(fromId);
    }
});

const stationData = Object.values(stations);
